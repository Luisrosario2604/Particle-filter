...
def draw_squares(img, particles_class, mode):
    ...
    if mode == "perturbation":
        ...
        particles_array = particles_class.get_particles_prediction()
        for particle in particles_array:
            x, y, w, h = particle.get_coords()

            cv2.rectangle(img, (x, y), (x + w, y + h), (51, 133, 255), 2)
    else:
        ...
        for particle in particles_array:
            ...
            if particle.get_score() > 0 and mode == "selection":
                cv2.rectangle(img, (x, y), (x + w, y + h), (102, 204, 0), 2)
...
def draw_all_images(filter_class, original_img, mask, i):
    ...
    draw_squares(all_squares_img, filter_class, "all")
    ...
    draw_squares(prediction_img, filter_class, "prediction")
    ...
    draw_title(all_squares_img, "Poblacion")
    ...
    tmp2 = cv2.hconcat([first_finded_img, all_squares_img, chosen_img])
    tmp3 = cv2.hconcat([selection_img, perturbation_img, prediction_img])
